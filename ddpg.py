...
class DDPG(object):
    ...
    def __init__(self, input_dims, buffer_size, hidden, layers, network_class, polyak, batch_size,
        ...
        """
        # Parameters used in the DDPG algorithm for clipping returns.
        # Clip:refers to limiting the returns to a specific range to prevent too large or too small a return value from adversely affecting the algorithm.
        ...
        # Convert input shapes to staging area based on input dimensions.
        input_shapes = dims_to_shapes(self.input_dims)
        self.dimo = self.input_dims['o']  # observation
        self.dimg = self.input_dims['g']  # goal
        self.dimu = self.input_dims['u']  # action

        # Prepare staging area for feeding data to the model.
        # staging are: buffer for storing data during the training process
        stage_shapes = OrderedDict()
        for key in sorted(self.input_dims.keys()):
            if key.startswith('info_'):
                continue
            # Stores shape information for each input
            stage_shapes[key] = (None, *input_shapes[key])
        # For inputs 'o' and 'g', a corresponding copy is also created for each input named 'o_2' and 'g_2'.
        # Because when using the HER algorithm, we need to use the target observation 'o' and the target goal 'g' in the training, whereas in the actual training, we need to input the current observation 'o_2' and the target goal 'g_2'.
        for key in ['o', 'g']:
            stage_shapes[key + '_2'] = stage_shapes[key]
        stage_shapes['r'] = (None,) # Adaptation to reward sequences of different lengths
        self.stage_shapes = stage_shapes

        # Create network.
        with tf.variable_scope(self.scope):
            # Used to store data into a buffer during the training
        ...
        # Configure the replay buffer --> to store and reuse samples of past experiences.
        # Set the number of time steps for each sample to T-1, since the past T-1 states are stored in the replay buffer
    ...
    # Auxiliary functions for generating randomized actions
    # n denotes the number of random actions generated
    def _random_action(self, n):
        return np.random.uniform(low=-self.max_u, high=self.max_u, size=(n, self.dimu))

    # Preprocessing states and targets
    ...
    # noise_eps: add Gaussian noise to actions to promote exploration
    # random_eps: Randomly select actions with a certain probability.
    # Whether to use the target policy network, the default is False, indicating that the primary policy network is used
    # Whether or not to compute the Q-value for the use of the TD3 algorithm in deterministic strategies, here the default is False
    def get_actions(self, o, ag, g, noise_eps=0., random_eps=0., use_target_net=False,
        ...
        # values to compute
        vals = [policy.pi_tf] # from Actor--> input observation & goal, output action
        if compute_Q:
            vals += [policy.Q_pi_tf] # from Critic --> input observation & goal, output corresponding action and Q-value
        ...
        u += noise # add random gaussian noise to the output action
        # action is clipped to between the specified ranges to ensure that the action does not exceed a reasonable range
        u = np.clip(u, -self.max_u, self.max_u)
        # introducing some exploratory behaviors via the ε-greedy strategy
    ...
    # initializes the demo buffer
    def init_demo_buffer(self, demoDataFile, update_stats=True): 

        demoData = np.load(demoDataFile) #load the demonstration data from data file
        # for each demo, break it down into observation, actions, desired_goal,achieved goal and info
        info_keys = [key.replace('info_', '') for key in self.input_dims.keys() if key.startswith('info_')]
        info_values = [np.empty((self.T - 1, 1, self.input_dims['info_' + key]), np.float32) for key in info_keys]

        # Add to global demo buffer
        demo_data_obs = demoData['obs']
        demo_data_acs = demoData['acs']
        demo_data_info = demoData['info']

        # initialize the whole demo buffer at the start of the training
        for epsd in range(self.num_demo): 
            obs, acts, goals, achieved_goals = [], [] ,[] ,[]
            i = 0
            # Get observations, actions, goals, and achieved goals for the corresponding time step from the demo data as iterations are performed in each demo
    ...
    # Used to synchronize the state of the optimizer (Adam optimizer).
    def _sync_optimizers(self):
        self.Q_adam.sync()
        self.pi_adam.sync()

    # Calculate and return the Critic loss, Actor loss, Critic's gradient and Actor's gradient in the batch
    def _grads(self):
    ...
    # Based on the gradient of Critic and the gradient of Actor, update the corresponding network parameters
